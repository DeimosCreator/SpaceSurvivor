...
namespace Editor
{
    ...
    public class ShipPartManagerEditor : UnityEditor.Editor
    {
        public override void OnInspectorGUI()
        {
            ...
            if (GUILayout.Button("Найти пути к gun02"))
            {
                string rootFolder = Application.dataPath + "/Resources/Sprites/Player/Parts";
                string[] allFiles = Directory.GetFiles(rootFolder, "gun02.prefab", SearchOption.AllDirectories);

                manager.availableGunPaths.Clear();
                ...
                foreach (string fullPath in allFiles)
                {
                    ...
                    manager.availableGunPaths.Add(relativePath);
                    Debug.Log("Найден путь: " + relativePath);
                }
